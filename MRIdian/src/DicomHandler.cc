...
void DicomHandler::SetDICOMFilesFolderPath(G4String filepath) /// we added this function to set up the file path for reading dicoom files-- BY Ruirui Liu
{
...
//     zslice->SetMinX(-fPixelSpacingX*fColumns/2.);// this is the G4 old version, just consider symetric geoemtry
...
//     zslice->SetMaxZ(fSliceLocation+fSliceThickness/2.);
    ...
    zslice->SetMinX(fImagePosX);
    zslice->SetMaxX(fImagePosX + fPixelSpacingX*(fColumns-1) );
    zslice->SetMinY(fImagePosY);
    zslice->SetMaxY(fImagePosY + fPixelSpacingY*(fRows-1));
    zslice->SetMinZ(fImagePosZ);
    zslice->SetMaxZ(fImagePosZ);
    
    
    
//     G4cout<<"-fPixelSpacingX*fColumns/2. "<<-fPixelSpacingX*fColumns/2.<<G4endl;
//     G4cout<<"-fPixelSpacingY*fRows/2. "<<-fPixelSpacingY*fRows/2.<<G4endl;
//     G4cout<<"fPixelSpacingX "<<fPixelSpacingX<<G4endl;
//     G4cout<<"fColumns/2. "<<fColumns/2.<<G4endl;
//     G4cout<<"fPixelSpacingY "<<fPixelSpacingY<<G4endl;
//     G4cout<<"fRows/2. "<<fRows/2.<<G4endl;
//     
//     G4cout<<"fSliceLocation is "<<fSliceLocation<<G4endl;
//     G4cout<<"fSliceThickness is "<<fSliceThickness<<G4endl;
    
    G4cout<<"fImagePosX "<<fImagePosX<<G4endl;
    G4cout<<"fImagePosY "<<fImagePosY<<G4endl;
    G4cout<<"fImagePosZ "<<fImagePosZ<<G4endl;
    
    G4cout<<"minX "<<zslice->GetMinX()<< " maxX "<<zslice->GetMaxX()<<G4endl;
    G4cout<<"minY "<<zslice->GetMinY()<< " maxY "<<zslice->GetMaxY()<<G4endl;
    G4cout<<"minZ "<<zslice->GetMinZ()<<" maxZ "<<zslice->GetMaxZ()<<G4endl;
...
//         fPixelSpacingY = atof( datas.substr(iss+2,datas.length()).c_str() ); 
	fPixelSpacingY = atof( datas.substr(iss+1,datas.length()).c_str() ); // here we change it to 1, seems something is wrong here
      ...
      std::vector<G4double>imagePosVec;
    
    while (end != std::string::npos) // WE USE THIS TO PARSE STRING TO GET IMAGE POSITION BY RL
    {
//         std::cout << s.substr(start, end - start) << std::endl;
	G4String tempStr = s.substr(start, end - start);
	imagePosVec.push_back(atof(tempStr.c_str()));
        start = end + delimiter.length();
        end = s.find(delimiter, start);
    }

//     std::cout << s.substr(start, end)<<endl;
      G4String tempStr = s.substr(start, end);
      imagePosVec.push_back(atof(tempStr.c_str()));
      
//       cout<<"print imagePosVec "<<endl;
//       for (int i = 0; i<imagePosVec.size(); i++)
//       {
// 	cout<<"imagePosVec "<<i<<" is "<<imagePosVec[i]<<endl;
//       }
      
      fImagePosX = imagePosVec[0];
      fImagePosY = imagePosVec[1];
      fImagePosZ = imagePosVec[2];
...
void DicomHandler::CheckFileFormat()
{
  ...
  // here we firstly tell CheckFileFormat where to look for the dicom files, and Data.dat file
  fDriverFile = dicomFilePath + "Data.dat";
  
  G4cout<<"Implementation CheckFileFormat"<<G4endl;
  std::ifstream checkData(fDriverFile.c_str());
  
//    cout<<"fDriverFile.c_str()"<<fDriverFile.c_str()<<endl;
//    exit(0);
  
  char * oneLine = new char[200]; // we increase this from 100 to read long fileanme, by RL
  
  if(!(checkData.is_open())) { //Check existance of Data.dat
    
    G4cout<<"If there is no existing Data.dat file, then we do following stuff"<<G4endl;
    G4String message = 
  ...
  checkData >> fCompression; // read the values in the Data.dat file, for example, here we put
  // the number in the first line of Data.dat file to fCompression
  
//   cout<<"fCompression "<<fCompression<<endl;
//   exit(0);
  
  G4cout<<"The fCompression is "<<fCompression<< G4endl;
  checkData >> fNFiles;
  G4cout<<"The fNFiles is "<<fNFiles<< G4endl;
  G4String oneName;
   checkData.getline(oneLine,100);
  G4cout<<"The onLine here is "<<oneLine<<G4endl;
...
//     cout<<"get here 1"<<endl;
    checkData.getline(oneLine,200);// increase this from 100 to 200 to read long filename, RL
    oneName = oneLine;
//     cout<<"oneName is "<<oneName<<endl;
//     exit(0);
//     G4cout<<"The onLine here is "<<oneLine<<G4endl;
    oneName += ".g4dcm"; // create dicomFile.g4dcm
//     G4cout<<"The oneName is "<<oneName<<G4endl; // this is the dicom file name

...
DicomDetectorConstruction::DicomDetectorConstruction()
 : G4VUserDetectorConstruction(),
   ...
   fConstructed(false)
{
  G4cout<<"This is the constructor of DicomDetectorConstruction"<<G4endl;
}
...
//....oooOO0OOooo........oooOO0OOooo........oooOO0OOooo.....
G4VPhysicalVolume* DicomDetectorConstruction::Construct() // function for building the geometry, where the phisical volume of world is defined
{
  
  cout<<"Implementation the Construct function in DicomDetectorConstruction"<<G4endl;
...
#ifdef G4_DCMTK
    ReadPhantomDataNew(); // if use DCMTK program, then use ReadPhantomDataNew to build geometry
    ConstructPhantomContainerNew();
#else
    ReadPhantomData(); // otherwise, use the regular one to build geometry
    ConstructPhantomContainer(); // this is the function for building the phantom geometry
#endif
    
    ConstructPhantom(); // default function, no phantom geometry is built
  }
    return fWorld_phys;
}

//....oooOO0OOooo........oooOO0OOooo........oooOO0OOooo........
void DicomDetectorConstruction::InitialisationOfMaterials() // function for initiliazing the mateirals
{
    // Creating elements :
...
//....oooOO0OOooo........oooOO0OOooo........oooOO0OOooo....
void DicomDetectorConstruction::ReadPhantomDataNew() // new function for reading the phantom using DCMTK
{
#ifdef G4_DCMTK
...
void DicomDetectorConstruction::ReadVoxelDensities( std::ifstream& fin ) //function for building the material density
{
  G4String stemp;
...
#else
   G4String dataFile = dicomFilePath + "Data.dat";
#endif
     cout<<"the dicomFilePath is "<<dicomFilePath<<endl;
     cout<<"the file of Data.dat is "<<dataFile<<endl;
//       exit(0);
...
//   exit(0);
  ...
  ReadPhantomDataFile(fname); // read all the dicom files through ReadPhantomDataFile function
...
void DicomDetectorConstruction::ReadPhantomDataFile(const G4String& fname)
{
  ...
  G4int nVoxels = sliceHeader->GetNoVoxels(); // this is the number of voxel in each slice, comment by RL
//   cout<<"nVoxels "<<nVoxels<<endl;
//   cout<<"fNoFiles "<<fNoFiles<<endl;
//   exit(0);
  ...
  if( fZSliceHeaders.size() == 1 ) 
  {
    ...
    fMateIDs = new size_t[fNoFiles*nVoxels];// declare the size for storing fMateIDs, size is the total number of voxels, comment by RL
  ...
  {
    fin >> mateID;
    fMateIDs[voxelCopyNo] = mateID; // comment: this is filling the material id to fMateIDs array, one voxel by one voxel
  }
  ...
  for( G4int ii = 0; ii < nVoxels; ii++, voxelCopyNo++ )// loop all the voxel in current slice: comment by RL
  {
    fin >> density;
    
    //-- Get material from list of original materials
//     cout<<"voxelCopyNo "<<voxelCopyNo<<endl;
    
    mateID = fMateIDs[voxelCopyNo];
    G4Material* mateOrig  = fOriginalMaterials[mateID];//comment: getting the actual material name by material index
...
//   exit(0);
//   cout<<"Finishing ReadPhantomDataFile "<<endl;
...
//   {
//     G4cout<<"the min z is "<<fZSliceHeaders[ii]->GetMinZ()<<G4endl;
//     G4cout<<"the min x is "<<fZSliceHeaders[ii]->GetMinX()<<G4endl;
//     G4cout<<"the min y is "<<fZSliceHeaders[ii]->GetMinY()<<G4endl;
//   }
//   
//   exit(0);
...
void DicomDetectorConstruction::ConstructPhantomContainer()
{
  ...
  fMinX = fZSliceHeaderMerged->GetMinX(); // get the min and max of X, Y, Z, by RL
  fMinY = fZSliceHeaderMerged->GetMinY();
  fMinZ = fZSliceHeaderMerged->GetMinZ();
  fMaxX = fZSliceHeaderMerged->GetMaxX();
  fMaxY = fZSliceHeaderMerged->GetMaxY();
  fMaxZ = fZSliceHeaderMerged->GetMaxZ();
...
//   exit(0);
  ...
  cout<<"xMin "<<fZSliceHeaderMerged->GetMinX()<<endl;
  cout<<"yMin "<<fZSliceHeaderMerged->GetMinY()<<endl;
  cout<<"zMin "<<fZSliceHeaderMerged->GetMinZ()<<endl;
  cout<<"xMax "<<fZSliceHeaderMerged->GetMaxX()<<endl;
  cout<<"yMaY "<<fZSliceHeaderMerged->GetMaxY()<<endl;
  cout<<"zMaZ "<<fZSliceHeaderMerged->GetMaxZ()<<endl;
  
  cout<<"The dimension information of the fContainer is: "<<endl;
  cout<<"x = "<<fNVoxelX*fVoxelHalfDimX<<endl;
  cout<<"y = "<<fNVoxelY*fVoxelHalfDimY<<endl;
  cout<<"z = "<<fNVoxelZ*fVoxelHalfDimZ<<endl;
  
		       
  cout<<"fOffsetX = "<<fOffsetX<<" fOffsetY = "<<fOffsetY<<" fOffsetZ = "<<fOffsetZ<<endl;
//   exit(0);
  fOffsetX = fOffsetX + 0*mm; // here we add some bais for avoiding the navigation warning, RL
  fOffsetY = fOffsetY + 0*mm;
  fOffsetZ = fOffsetZ + 0*mm;
...
// 	G4double gantryRtn = 50*deg; // here we add a tiny rotation for the phantom
// 	G4double collRtn = 10*deg;
//       rotmBtmp.rotateX(gantryRtn);
//       rotmBtmp.rotateZ(collRtn);
  
  G4Transform3D transform = G4Transform3D(rotmBtmp,posCentreVoxels);
...
G4ThreeVector DicomDetectorConstruction::GetVoxelNumberXYZ()
{
...
//     G4double fMaxX,fMaxY,fMaxZ; // maximum extension of voxels (position of wall)
  G4ThreeVector fNVoxelVector = G4ThreeVector(fNVoxelX,fNVoxelY,fNVoxelZ);
  return fNVoxelVector;
  
}

G4ThreeVector DicomDetectorConstruction::GetVoxelDimension()
{
//   cout<<"fVoxelHalfDimX = "<<fVoxelHalfDimX<<" fVoxelHalfDimY = "<<fVoxelHalfDimY<<" fVoxelHalfDimZ ="<<fVoxelHalfDimZ<<endl;
//   exit(0);
  G4ThreeVector fVoxelDimVector = G4ThreeVector(fVoxelHalfDimX*2,fVoxelHalfDimY*2,fVoxelHalfDimZ*2);
  return fVoxelDimVector;

}

G4ThreeVector DicomDetectorConstruction::GetVoxelDimMinXYZ()
{
//     cout<<"fMinX = "<<fMinX<<" fMinY = "<<fMinY<<" fMinZ ="<<fMinZ<<endl;
//  exit(0);
  G4ThreeVector fVoxelDimMinVector = G4ThreeVector(fMinX,fMinY,fMinZ);
  return fVoxelDimMinVector;
}

G4ThreeVector DicomDetectorConstruction::GetVoxelDimMaxXYZ()
{
//       cout<<"fMaxX = "<<fMaxX<<" fMaxY = "<<fMaxY<<" fMaxZ ="<<fMaxZ<<endl;
//  exit(0);
  G4ThreeVector fVoxelDimMaxVector = G4ThreeVector(fMaxX,fMaxX,fMaxZ);
  return fVoxelDimMaxVector;

}

void DicomDetectorConstruction::SetDICOMFilesFolderPath(G4String filepath)
{

...
void CellLayoutInitializer::RectangularSlabFullMesh(double xDim, double yDim, double zDim)
{
    ...
    int effectiveCellNumber =N_X*N_Y*N_Z;
...
//     cout<<"the size of cellPositionX now is "<<cellPositionX.size()<<endl;
	...
	for (int i = 0; i<N_X; i++)
	{
	  ...
	  tempIndex.j = j;
	  tempIndex.k = k;
	  int n = N_X*N_Y*k + N_Y*i + j;
	  positionIndex[n] = tempIndex;
	}
      }
    }
...
void CellLayoutInitializer::BlobShell(double dDim1, double dDim2, int seededCellNumber)
{
    double xDim;
    double yDim;
    double zDim;
    ...
    for (int i =0;i<N_X;i++)
    {
        for (int j =0;j<N_Y;j++) 
        {
            for (int k =0;k<N_Z;k++)
            {
                ...
                y=-1*yDim/2+j*cellHomeSizeY+cellHomeSizeY/2;
                z=-z*zDim/2+k*cellHomeSizeZ+cellHomeSizeZ/2;
// 		cout<<"0.25*dDim2*dDim2="<<0.25*dDim2*dDim2<<endl;
// 		cout<<"0.25*dDim1*dDim1="<<0.25*dDim1*dDim1<<endl;
// 		cout<<"x*x+y*y+z*z="<<x*x+y*y+z*z<<endl;
                if ((x*x+y*y+z*z>0.25*dDim2*dDim2)&&(x*x+y*y+z*z<0.25*dDim1*dDim1))
                {
                    totalCellInsideRegion = totalCellInsideRegion + 1;     
// 		    cout<<"select one"<<endl;
    ...
    cout<<"the effectiveCellNumber is "<<effectiveCellNumber<<endl;
    cout<<"the totalCellInsideRegion is "<<totalCellInsideRegion<<endl;
    ...
    while (count<effectiveCellNumber)
    {
        ...
        y=-1*yDim/2+jt*cellHomeSizeY+cellHomeSizeY/2;
        z=-1*zDim/2+kt*cellHomeSizeZ+cellHomeSizeZ/2;
        if ((cellHomeResidence[kt][it][jt]==0)&&(x*x+y*y+z*z<0.25*dDim1*dDim1)&& (x*x+y*y+z*z>0.25*dDim2*dDim2))
        {
            cellHomeResidence[kt][it][jt]=1;
            Index tempIndex;
            tempIndex.i=it;
            tempIndex.j=jt;
            tempIndex.k=kt;
            positionIndex[count]=tempIndex; // asign position index for positionIndex vector
            count=count+1;
        } 
